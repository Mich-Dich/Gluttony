#pragma once

// Unsigned int types.
typedef unsigned char u8;
typedef unsigned short u16;
typedef unsigned int u32;
typedef unsigned long long u64;

// Signed int types.
typedef signed char int8;
typedef signed short int16;
typedef signed int int32;
typedef signed long long int64;

// Floating point types
//typedef float f32;
//typedef double f64;

// Boolean types
typedef int bool32;
typedef char bool8;

// Properly define static assertions.
#if defined(__clang__) || defined(__gcc__)
    #define STATIC_ASSERT _Static_assert
#else
    #define STATIC_ASSERT static_assert
#endif

// Ensure all types are of the correct size.
STATIC_ASSERT(sizeof(u8) == 1, "Expected u8 to be 1 byte.");
STATIC_ASSERT(sizeof(u16) == 2, "Expected u16 to be 2 bytes.");
STATIC_ASSERT(sizeof(u32) == 4, "Expected u32 to be 4 bytes.");
STATIC_ASSERT(sizeof(u64) == 8, "Expected u64 to be 8 bytes.");

STATIC_ASSERT(sizeof(int8) == 1, "Expected i8 to be 1 byte.");
STATIC_ASSERT(sizeof(int16) == 2, "Expected i16 to be 2 bytes.");
STATIC_ASSERT(sizeof(int32) == 4, "Expected i32 to be 4 bytes.");
STATIC_ASSERT(sizeof(int64) == 8, "Expected i64 to be 8 bytes.");

STATIC_ASSERT(sizeof(float) == 4, "Expected f32 to be 4 bytes.");
STATIC_ASSERT(sizeof(double) == 8, "Expected f64 to be 8 bytes.");

#define TRUE 1
#define FALSE 0

// Platform detection
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) 
    #define GL_PLATFORM_WINDOWS 1
#ifndef _WIN64
    #error "64-bit is required on Windows!"
#endif

// Linux OS
#elif defined(__linux__) || defined(__gnu_linux__)
    #define GL_PLATFORM_LINUX 1
#if defined(__ANDROID__)
    #define GL_PLATFORM_ANDROID 1
#endif

// Catch anything not caught by the above.
#elif defined(__unix__)
    #define GL_PLATFORM_UNIX 1
    
// Posix
#elif defined(_POSIX_VERSION)
    #define GL_PLATFORM_POSIX 1
    
// Apple platforms
#elif __APPLE__
    #define GL_PLATFORM_APPLE 1
    #include <TargetConditionals.h>
    
    // iOS Simulator
    #if TARGET_IPHONE_SIMULATOR
        #define GL_PLATFORM_IOS 1
        #define GL_PLATFORM_IOS_SIMULATOR 1
        
    // iOS device
    #elif TARGET_OS_IPHONE
        #define GL_PLATFORM_IOS 1

    // Other kinds of Mac OS
    #elif TARGET_OS_MAC
    #else
        #error "Unknown Apple platform"
    #endif
#else
    #error "Unknown platform!"
#endif

// Exports
#ifdef GL_EXPORT
    #ifdef _MSC_VER
        #define GLUTTONY_API __declspec(dllexport)
    #else
        #define GLUTTONY_API __attribute__((visibility("default")))
    #endif
#else
// Imports
#ifdef _MSC_VER
        #define GLUTTONY_API __declspec(dllimport)
    #else
        #define GLUTTONY_API
    #endif
#endif