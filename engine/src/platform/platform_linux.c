...
// Linux platform layer.
    ...
    bool8 platform_Startup(
        ...
        if (stream_result <= 0) {
            GL_FATAL("An error occurred when flusing the stream: %d", stream_result);
            return FALSE;
        }
    ...
    void platform_shutdown(platformState* state) {
        ...
        PLatformInternalState* state = (PLatformInternalState*)state->internal_state;
    ...
    bool8 platform_Push_messages(platformState* state) {
        ...
        // Simply cold-cast to the known type.
        PLatformInternalState* state = (PLatformInternalState*)state->internalState;
